...
export const messageService = {
  ...
  async create(input) {
    ...
    await messageRepository.create(message, { prepend: true });
  ...
  async setRead(id, read) {
    const updated = await messageRepository.update(id, { read: Boolean(read) });
    if (!updated) throw notFound('Message');
    return updated;
  },

  async remove(id) {
    if (!(await messageRepository.remove(id))) throw notFound('Message');
